...
Express 5 + TypeScript vehicle-rental backend (Knex/Postgres, JWT auth, Joi validation, Multer/Cloudinary uploads). Feature layer is in `src/modules/` (routes/controllers/services per feature) — `auth`, `vehicles`, and `rentals` are built; `reports` remains.
...
- `src/modules/` — one folder per feature (`auth`, `vehicles`, `rentals` built; `reports` empty). Each has `X.route.ts` / `X.controller.ts` / `X.service.ts` / `X.validation.ts`. **Services are classes with a singleton default export** (`export default new XService()`) — controllers call the instance, never functions. Services use the shared knex instance from `src/config/database.ts`; controllers call services and send `{ success, message, data }`; Express 5 auto-forwards async rejections, so controllers use `try/catch → next(error)`.
