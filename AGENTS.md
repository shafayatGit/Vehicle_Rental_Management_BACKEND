...
Express 5 + TypeScript vehicle-rental backend (Knex/Postgres, JWT auth, Joi validation, Multer/Cloudinary uploads). Feature layer is in `src/modules/` (routes/controllers/services per feature) — `auth`, `vehicles`, `rentals`, and `reports` are all built.
...
- `src/middleware/` — `auth.middleware.ts` (Bearer JWT → `req.user`, throws 401), `validate.middleware.ts` (Joi schema → 400 with `errorSources`; takes a `source` arg — `"body"` (default) | `"query"` | `"params"` — and writes the validated value back to `req`), `upload.middleware.ts` (multer + `CloudinaryStorage`, image-only `fileFilter`, no size limit, single field `"photo"`), `rateLimit.middleware.ts` (express-rate-limit, 20 attempts / 15 min per IP, wired only on `POST /auth/login`), `globalErrorHandler.ts` (maps `AppError` → its status/message; `MulterError` → 400; dev vs prod envelope), `notFound.ts`.
- `src/modules/` — one folder per feature (`auth`, `vehicles`, `rentals`, `reports` built). Each has `X.route.ts` / `X.controller.ts` / `X.service.ts` / `X.validation.ts`. **Services are classes with a singleton default export** (`export default new XService()`) — controllers call the instance, never functions. Services use the shared knex instance from `src/config/database.ts`; controllers call services and send `{ success, message, data }`; Express 5 auto-forwards async rejections, so controllers use `try/catch → next(error)`.
...
- `src/modules/rentals/` — `list` (paginated, `vehicle_id`/`status`/date-range overlap filters), `getById`, `create`/`update` (server-side `total_amount` = vehicle `daily_rate` × days, same start/end = 1 day), `remove` (hard delete). **Overlap rule:** 409 if any non-`cancelled` rental on the vehicle overlaps `[start_date, end_date]` (`start1 <= end2 AND end1 >= start2`); cancelled rentals never block. `create` runs inside a transaction that **locks the vehicle row** (`SELECT ... FOR UPDATE`) before the overlap check, so concurrent double-bookings can't both succeed; `update` also uses a transaction and re-checks overlap when dates/vehicle change (excluding itself). Dates are strict `YYYY-MM-DD` strings (rejects e.g. Feb 30) and `end_date >= start_date`. Routes: `GET /rentals`, `GET /rentals/:id`, `POST /rentals`, `PUT /rentals/:id` (partial update, can set `status`), `DELETE /rentals/:id` — all JWT-protected.
- `src/modules/reports/` — `GET /reports/rentals?month=YYYY-MM&vehicle_id=` — monthly report per active (non-`cancelled`) vehicle of `total_bookings`, `days_rented`, `revenue` with **proration**: only days inside the requested month count (`LEAST(end, month_end) - GREATEST(start, month_start) + 1`), so a rental spanning Jul 29–Aug 3 contributes 3 days to August. Excludes soft-deleted vehicles; `month` defaults to the current month, `vehicle_id` is optional; response also includes `topVehicle` (highest revenue; ties broken by `id`). Route is JWT-protected.
